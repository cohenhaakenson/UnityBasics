...
public partial class CamSupport : MonoBehaviour
{
    public void MoveBy(float dx, float dy)
    {
        ...
        positionLerp.BeginLerp(transform.position, p);
    }
    ...
    public void Zoom(float zoom)
    {
        ...
        {
            Vector2 b, e;
            b.x = theCam.orthographicSize;
            b.y = 0f;
            e.x = theCam.orthographicSize * zoom;
            e.y = 0f;
            sizeLerp.BeginLerp(b, e);
        }
    }
    ...
    {
        shake.SetShakeParameters(frequency, durationSeconds);
    }

    public void ShakeCamera(Vector2 delta)
    {
        shake.SetShakeMagnitude(delta, transform.position);
    }
    
    public void SetLerpParameters(float duration, float rate)
    {
        positionLerp.SetLerpParms(duration, rate);
        sizeLerp.SetLerpParms(duration, rate);      // Similar values
    }
